...
class MathDisplayUtils {
  ...
  static String toLatexFormat(String expression) {
    ...
    latex = latex.replaceAllMapped(RegExp(r'([a-zA-Z0-9]+)/([a-zA-Z0-9]+)'),
        (match) {
      return r'\frac{' + match.group(1)! + r'}{' + match.group(2)! + r'}';
    });
    ...
    latex = latex.replaceAllMapped(
        RegExp(r'\b(sin|cos|tan|ln|log|exp|abs|gamma|lim|det)\b'), (match) {
    ...
    // Convert limits
    latex = latex.replaceAllMapped(
        RegExp(r'limit\(([^,]+),\s*([a-zA-Z]),\s*([^)]+)\)'), (match) {
      return r'\lim_{' +
          match.group(2)! +
          r' \to ' +
          match.group(3)! +
