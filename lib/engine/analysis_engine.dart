...
//
// Curve-sketching analysis built on top of CalculatorEngine. Heavy on
// SymEngine result normalization because the bridge returns strings in
// SymEngine's preferred format (complex numbers, ** for power, etc).
...
class AnalysisEngine {
    ...
    try {
      ...
      if (variable != null && value != null) {
        ...
        expr = expr.replaceAll(
            RegExp(r'\b' + RegExp.escape(variable) + r'\b'), '($value)');
      }

      final result = _engine.evaluate(expr);
      return _normalizeResult(result);
    } catch (e) {
      debugPrint('EVAL_ERROR: Failed to evaluate "$expression": $e');
      return 'Error';
    }
  }
  ...
  String _safeDifferentiate(String expression, String variable) {
    try {
      debugPrint('DIFF: Differentiating "$expression" w.r.t. $variable');
      final result = _engine.differentiate(expression, variable);
      debugPrint('DIFF: Raw result: "$result"');
      final normalized = _normalizeResult(result);
      debugPrint('DIFF: Normalized result: "$normalized"');
      return normalized;
    } catch (e) {
      debugPrint('DIFF_ERROR: Failed to differentiate "$expression": $e');
      return 'Error';
    }
  }
  ...
  String _safeSolveRaw(String expression, String variable) {
    try {
      ...
      debugPrint('SOLVE_RAW: Engine returned: "$result"');
      ...
      debugPrint('SOLVE_RAW_ERROR: Failed to solve "$expression": $e');
  ...
  List<String> _parseSolutionArray(String solution) {
    ...
    debugPrint('PARSE_ARRAY: Input: "$solution"');

    try {
      if (solution.startsWith('[') && solution.endsWith(']')) {
        final content = solution.substring(1, solution.length - 1).trim();
        debugPrint('PARSE_ARRAY: Content after bracket removal: "$content"');
        ...
        debugPrint('PARSE_ARRAY: Final parsed parts: $parts');
        return parts;
      }
      ...
      if (!solution.contains('Error')) {
        ...
        debugPrint('PARSE_ARRAY: Single solution normalized: "$normalized"');
        return [normalized];
      }
    } catch (e) {
      debugPrint('PARSE_ERROR: Failed to parse solution "$solution": $e');
    }
  ...
  String _determineExtremumType(String secondDerivative, String point) {
    final concavityStr =
        _safeEvaluate(secondDerivative, variable: 'x', value: point);
  ...
  Future<AnalysisResult> performCurveAnalysis(String function) async {
    debugPrint('ANALYSIS: Starting analysis of: "$function"');
    ...
    debugPrint('ANALYSIS: Calculating derivatives...');
    ...
    debugPrint('ANALYSIS: Finding Y-intercept...');
    ...
    if (firstDerivative != 'Error') {
      ...
      if (criticalPointsRaw.isEmpty) {
        ...
        for (final pointRaw in criticalPointsRaw) {
          debugPrint(
              'ANALYSIS: Evaluating function at critical point: "$pointRaw"');
          // Use the raw value for evaluation
          final functionValue =
              _safeEvaluate(function, variable: 'x', value: pointRaw);
          debugPrint(
              'ANALYSIS: Function value at x=$pointRaw: "$functionValue"');
          ...
          if (functionValue != 'Error') {
            ...
            errors
                .add('Failed to evaluate function at critical point $pointRaw');
          }
        }
      }
    } else {
    ...
    debugPrint('ANALYSIS: Finding inflection points...');
    ...
    if (secondDerivative != 'Error') {
      ...
      debugPrint(
          'ANALYSIS: Found ${inflectionPointsRaw.length} potential inflection points: $inflectionPointsRaw');
      ...
      if (inflectionPointsRaw.isEmpty) {
        ...
        if (constCheck != null) {
          if (constCheck == 0) {
            ...
            inflectionPoints
                .add('No inflection points (f\'\'(x) = $secondDerivative ≠ 0)');
          }
        } else {
        ...
        for (final pointRaw in inflectionPointsRaw) {
          debugPrint(
              'ANALYSIS: Evaluating function at inflection point: "$pointRaw"');
          // Use the raw value for evaluation
          final functionValue =
              _safeEvaluate(function, variable: 'x', value: pointRaw);
          debugPrint(
          ...
          if (functionValue != 'Error') {
            ...
            errors.add(
                'Failed to evaluate function at inflection point $pointRaw');
          }
        }
      }
    } else {
    ...
    debugPrint('ANALYSIS: Analysis complete. Errors: ${errors.length}');

    return AnalysisResult(
      originalFunction: function,
      firstDerivative: firstDerivative != 'Error'
          ? _normalizeForDisplay(firstDerivative)
          : 'Error',
      secondDerivative: secondDerivative != 'Error'
          ? _normalizeForDisplay(secondDerivative)
          : 'Error',
      yIntercept:
          yIntercept != 'Error' ? _normalizeForDisplay(yIntercept) : 'Error',
      // Enhanced root formatting with LaTeX
      roots: rootsRaw.isEmpty
          ? ['No real roots found']
          : rootsRaw.map((r) {
      ...
      inflectionPoints: inflectionPoints.map((point) {
        ...
        return point.replaceAllMapped(RegExp(r'\(([^,]+),\s*([^)]+)\)'),
            (match) {
          final x = MathDisplayUtils.formatMathResult(match.group(1)!);
          final y = MathDisplayUtils.formatMathResult(match.group(2)!);
          return '($x, $y)';
