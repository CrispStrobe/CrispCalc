...
| **Web** (Vercel) | ◐ pure-Dart only | **Live: https://crisp-calc.vercel.app** (PLAN P10 Path A). Calculator, statistics, matrices, Sudoku/CSP, units work — plus a pure-Dart CAS interim: numeric eval, and `expand` / `differentiate` / `solve` for single-variable polynomials (linear + quadratic, incl. surd & complex roots). Full symbolic CAS / MPFR precision / FLINT number-theory still need the desktop or mobile app, pending the SymEngine→WASM bridge (no `dart:ffi` in the browser). |
